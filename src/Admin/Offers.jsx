...
const validateForm = (form, isReactivate = false) => {
  ...
  if (!form.startDate) errs.startDate = "Start date is required.";
    ...
    errs.startDate =
      "Start date must be today or in the future when reactivating.";
...
const OfferForm = ({
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const fieldMap = {
          ...
          description:
            serverErrs.Description?.[0] || serverErrs.description?.[0],
          discountPercentage:
            serverErrs.DiscountPercentage?.[0] ||
            serverErrs.discountPercentage?.[0],
  ...
  const labels = {
    add: {
      title: "New Offer",
      sub: "Fill in the details to create a new offer.",
    },
    edit: { title: "Edit Offer", sub: "Update the offer details below." },
    reactivate: {
      title: "Reactivate Offer",
      sub: "Update dates to bring this expired offer back.",
    },
  };

  return (
    <div
      className={`off-card-edit-panel${isReactivate ? " off-card-edit-panel--reactivate" : ""}`}
    >
      {success ? (
        <div
          style={{
        ...
        <>
          ...
          {apiError && (
            <div
              key={apiError}
              className="off-alert off-alert--error"
          ...
          <div className="off-form-grid">
            {/* Title */}
            <div
              className={`off-form-field off-form-field--full${fieldErrors.title ? " has-error" : ""}`}
            >
              <label htmlFor="off-title">
                Title <span style={{ color: "#D0392B" }}>*</span>
              </label>
              <input
                id="off-title"
                name="title"
              ...
              />
              {fieldErrors.title && (
                <span className="off-field-error">{fieldErrors.title}</span>
              )}
            </div>

            {/* Description */}
            <div
              className={`off-form-field off-form-field--full${fieldErrors.description ? " has-error" : ""}`}
            >
              ...
              {fieldErrors.description && (
                <span className="off-field-error">
                  {fieldErrors.description}
                </span>
              )}
            </div>
            ...
            {/* Discount */}
            <div
              className={`off-form-field${fieldErrors.discountPercentage ? " has-error" : ""}`}
            >
              <label htmlFor="off-pct">
                Discount % <span style={{ color: "#D0392B" }}>*</span>
              </label>
              <input
                id="off-pct"
            ...
            {/* Start date */}
            <div
              className={`off-form-field${fieldErrors.startDate ? " has-error" : ""}`}
            >
              <label htmlFor="off-start">
                Start Date <span style={{ color: "#D0392B" }}>*</span>
              </label>
              <input
                id="off-start"
                name="startDate"
              ...
              {fieldErrors.startDate && (
                <span className="off-field-error">{fieldErrors.startDate}</span>
              )}
            </div>

            {/* End date */}
            <div
              className={`off-form-field${fieldErrors.endDate ? " has-error" : ""}`}
            >
              <label htmlFor="off-end">
                End Date <span style={{ color: "#D0392B" }}>*</span>
              </label>
              <input
                id="off-end"
                name="endDate"
              ...
              />
              {fieldErrors.endDate && (
                <span className="off-field-error">{fieldErrors.endDate}</span>
              )}
            </div>
          </div>
  ...
  <div className="off-modal-overlay" onClick={onCancel}>
    <div className="off-modal" onClick={(e) => e.stopPropagation()}>
      ...
      <p className="off-modal-msg">
        ...
        <span className="off-modal-name">"{offer.title}"</span>. This action
        cannot be undone.
      </p>
      {error && (
        <div
      ...
      <div className="off-modal-actions">
        <button
          className="off-modal-cancel-btn"
          onClick={onCancel}
          disabled={loading}
        >
          Cancel
        ...
        <button
          className="off-modal-delete-btn"
          onClick={onConfirm}
          disabled={loading}
        >
      ...
      <div className="off-card">
        ...
        <div className="off-card-info">
          ...
          {offer.description ? (
            ...
            <p className="off-card-desc off-card-desc--empty">
  ...
  return (
    <div className={`off-section off-section--${type}`}>
      <div className="off-section-header">
        <div className={`off-section-badge off-section-badge--${type}`}>
          {icon}
        </div>
...
export default function Offers() {
  ...
  const fetchCurrent = useCallback(async () => {
    ...
    setErrorCurrent("");
    try {
      const res = await fetch(`${API}/Offers/Offers`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json().catch(() => []);
      if (!res.ok) {
        setErrorCurrent("Failed to load current offers.");
        return;
      }
      setCurrent(Array.isArray(data) ? data : []);
    } catch {
      setErrorCurrent("Network error loading current offers.");
    } finally {
  ...
  const fetchUpcoming = useCallback(async () => {
    ...
    setErrorUpcoming("");
    try {
      const res = await fetch(`${API}/Offers/UpcomingOffers`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json().catch(() => []);
      if (!res.ok) {
        setErrorUpcoming("Failed to load upcoming offers.");
        return;
      }
      setUpcoming(Array.isArray(data) ? data : []);
    } catch {
      setErrorUpcoming("Network error loading upcoming offers.");
    } finally {
  ...
  const fetchExpired = useCallback(async () => {
    ...
    setErrorExpired("");
    try {
      const res = await fetch(`${API}/Offers/ExpiredOffers`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      const data = await res.json().catch(() => []);
      if (!res.ok) {
        setErrorExpired("Failed to load expired offers.");
        return;
      }
      setExpired(Array.isArray(data) ? data : []);
    } catch {
      setErrorExpired("Network error loading expired offers.");
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setDeleteError(extractServerError(data));
        return;
      }
