...
/* ── Generic delete modal (used for both doctors and receptionists) ─────────── */
const DeleteModal = ({ title, name, onConfirm, onCancel, loading }) => (
...
const HospitalInfo = () => {
  ...
  // Doctor reactivate
  const [docReactivatingId, setDocReactivatingId]   = useState(null);
  const [docReactivateError, setDocReactivateError] = useState("");
  ...
  const [recDeleteLoading, setRecDeleteLoading] = useState(false);
  const [recDeleteError, setRecDeleteError]     = useState("");

  // Receptionist reactivate
  const [recReactivatingId, setRecReactivatingId]   = useState(null);
  const [recReactivateError, setRecReactivateError] = useState("");
  ...
  useEffect(() => {
    ...
    fetch(`${API}/Hospital/dashboard/${hospitalId}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(async res => {
  ...
  const fetchDoctors = useCallback(() => {
    ...
    fetch(`${API}/Hospital/${hospitalId}/doctors?${params}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(async res => {
        if (res.status === 401) { navigate("/login"); return; }
  ...
  const fetchReceps = useCallback(() => {
    ...
    fetch(`${API}/Hospital/${hospitalId}/receptionists?${params}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(async res => {
        if (res.status === 401) { navigate("/login"); return; }
  ...
  const handleDocDeleteConfirm = async () => {
    ...
    setDocDeleteLoading(true); setDocDeleteError("");
    try {
      const res = await fetch(`${API}/Admin/Doctor/${docDeleteTarget.id}`, {
        method: "DELETE",
      ...
      if (!res.ok) {
        ...
        setDocDeleteError(errs.Identity?.[0] || errs.identity?.[0] || data.message || "Delete failed.");
        return;
      }
      setDocDeleteTarget(null);
      fetchDoctors();
    } catch {
      setDocDeleteError("Server error. Please try again.");
    } finally {
  ...
  // ── Doctor reactivate ─────────────────────────────────────────────────────
  const handleDocReactivate = async (doctorId) => {
    setDocReactivatingId(doctorId); setDocReactivateError("");
    try {
      const res = await fetch(`${API}/Admin/reactive-account/${doctorId}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.status === 401) { navigate("/login"); return; }
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        const errs = data.errors || data.Errors || {};
        setDocReactivateError(errs.Identity?.[0] || errs.identity?.[0] || data.message || "Reactivation failed.");
        return;
      }
      fetchDoctors();
    } catch {
      setDocReactivateError("Server error. Please try again.");
    } finally {
      setDocReactivatingId(null);
    }
  };

  // ── Receptionist delete ───────────────────────────────────────────────────
  const handleRecDeleteConfirm = async () => {
    if (!recDeleteTarget) return;
    setRecDeleteLoading(true); setRecDeleteError("");
    try {
      const res = await fetch(`${API}/Admin/Receptionist/${recDeleteTarget.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.status === 401) { navigate("/login"); return; }
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        const errs = data.errors || data.Errors || {};
        setRecDeleteError(errs.Identity?.[0] || errs.identity?.[0] || data.message || "Delete failed.");
        return;
      }
      setRecDeleteTarget(null);
      fetchReceps();
    } catch {
      setRecDeleteError("Server error. Please try again.");
    } finally {
      setRecDeleteLoading(false);
    }
  };

  // ── Receptionist reactivate ───────────────────────────────────────────────
  const handleRecReactivate = async (recepId) => {
    setRecReactivatingId(recepId); setRecReactivateError("");
    try {
      const res = await fetch(`${API}/Admin/reactive-account/${recepId}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.status === 401) { navigate("/login"); return; }
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        const errs = data.errors || data.Errors || {};
        setRecReactivateError(errs.Identity?.[0] || errs.identity?.[0] || data.message || "Reactivation failed.");
        return;
      }
      fetchReceps();
    } catch {
      setRecReactivateError("Server error. Please try again.");
    } finally {
      setRecReactivatingId(null);
    }
  };
  ...
  const handleDeleteHospital = async () => {
    ...
    try {
      ...
      setHospError("Server error. Please try again later.");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="hosp-page">
      <div className="hosp-inner">
        ...
        <div className="hosp-panel">
          ...
          {docDeleteError    && <div className="hosp-alert" style={{ margin: "0.75rem 1.25rem 0" }}>{docDeleteError}</div>}
          {docReactivateError && <div className="hosp-alert" style={{ margin: "0.75rem 1.25rem 0" }}>{docReactivateError}</div>}
              ...
              <table className="hosp-table">
                ...
                <tbody>
                  {doctors.map(doc => {
                    ...
                    const isReactivating = docReactivatingId === doc.id;
                    return (
                        ...
                        <td>
                          <div className="hosp-row-actions">
                            ...
                            <button
                              ...
                              onClick={() => { setDocDeleteError(""); setDocReactivateError(""); setDocDeleteTarget({ id: doc.id, name: fullName }); }}
                            >
                            ...
                            >
                              {isReactivating ? <span className="hosp-spinner hosp-spinner--dark" /> : (
                                <>
                                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="14" height="14"><polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" /></svg>
                                  Reactivate
                                </>
                              )}
                            </button>
        ...
        <div className="hosp-panel">
          ...
          {recDeleteError    && <div className="hosp-alert" style={{ margin: "0.75rem 1.25rem 0" }}>{recDeleteError}</div>}
          {recReactivateError && <div className="hosp-alert" style={{ margin: "0.75rem 1.25rem 0" }}>{recReactivateError}</div>}
          ...
          <div className="hosp-table-wrap">
            {recLoading ? <TableSkeleton rows={4} /> : recError ? (
              ...
              <table className="hosp-table">
                <thead>
                  {/* Same 4 columns as doctors */}
                  <tr><th>Receptionist</th><th>Gender</th><th>Status</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  {receps.map(rec => {
                    ...
                    const isDeleted      = !!rec.isDeleted;
                    const isActive       = !!rec.isActive;
                    const isReactivating = recReactivatingId === rec.id;

                    return (
                      <tr key={rec.id} className={isDeleted ? "hosp-row--deleted" : ""}>
                        {/* Name + avatar */}
                        <td>
                          <div className="hosp-doc-cell">
                            <div className="hosp-doc-avatar">
                              {rec.profileImageUrl
                                ? <img src={rec.profileImageUrl} alt={fullName} />
                                : <span className="hosp-doc-avatar-initials">{buildInitials(firstName, lastName)}</span>}
                            </div>
                            <span className="hosp-doc-name">{fullName}</span>
                          </div>
                        </td>
                        ...
                        {/* Gender */}
                        <td>
                          <span className={`hosp-badge hosp-badge--${(rec.gender || "").toLowerCase()}`}>
                            {rec.gender || "—"}
                          </span>
                        </td>
                        ...
                        <td>
                          <div className="hosp-row-actions">
                            <button
                              className="hosp-action-btn hosp-action-btn--delete"
                              disabled={isDeleted}
                              title={isDeleted ? "Receptionist is already deleted" : "Delete receptionist"}
                              onClick={() => { setRecDeleteError(""); setRecReactivateError(""); setRecDeleteTarget({ id: rec.id, name: fullName }); }}
                            >
                              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /><path d="M10 11v6M14 11v6" /><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2" /></svg>
                              Delete
                            </button>
                            <button
                              className="hosp-action-btn hosp-action-btn--reactivate"
                              disabled={!isDeleted || isReactivating}
                              title={!isDeleted ? "Receptionist is not deleted" : "Reactivate this receptionist"}
      ...
      {docDeleteTarget && (
        <DeleteModal
          ...
          onCancel={() => { if (!docDeleteLoading) { setDocDeleteTarget(null); setDocDeleteError(""); } }}
      ...
      {recDeleteTarget && (
        <DeleteModal
          ...
          onCancel={() => { if (!recDeleteLoading) { setRecDeleteTarget(null); setRecDeleteError(""); } }}
          loading={recDeleteLoading}
        />
      )}
    </div>
  );
