...
const StatCard = ({ icon, value, label, sub, colorClass, fillPct }) => {
  ...
  const barClass =
    pct >= 90
      ? "hosp-stat-bar-fill--high"
      : pct >= 70
        ? "hosp-stat-bar-fill--medium"
        : "";
  return (
    <div className={`hosp-stat-card hosp-stat-card--${colorClass}`}>
      <div className={`hosp-stat-icon hosp-stat-icon--${colorClass}`}>
        {icon}
      </div>
      <div>
  ...
  <div className="hosp-stats-grid">
    {[1, 2, 3, 4].map((i) => (
      <div key={i} className="hosp-stat-card hosp-stat-card--blue">
        <div
          className="hosp-skeleton hosp-skeleton-circle"
          style={{ width: 42, height: 42, borderRadius: 10 }}
        />
        <div>
          <div
            className="hosp-skeleton hosp-skeleton-line"
...
const HospitalInfo = () => {
  ...
  useEffect(() => {
    ...
    setDashLoading(true);
    setDashError("");
    fetch(`${API}/Hospital/dashboard/${hospitalId}`, {
  ...
  const fetchDoctors = useCallback(() => {
    ...
    fetch(`${API}/Hospital/${hospitalId}/doctors?${params}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(async (res) => {
        if (res.status === 401) {
        ...
        if (!res.ok) {
          setDocError("Failed to load doctors.");
          return;
        }
        ...
        setDocTotal({
          ...
          items: response.totalRecords ?? 0,
          size: response.pageSize ?? 15,
        });
      })
  ...
  const fetchReceps = useCallback(() => {
    ...
    fetch(`${API}/Hospital/${hospitalId}/receptionists?${params}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(async (res) => {
        if (res.status === 401) {
        ...
        if (!res.ok) {
          setRecError("Failed to load receptionists.");
          return;
        }
  ...
  const handleDocReactivate = async (doctorId) => {
    setDocReactivatingId(doctorId);
    setDocReactivateError("");
    try {
  ...
  const handleRecDeleteConfirm = async () => {
    ...
    setRecDeleteLoading(true);
    setRecDeleteError("");
    try {
      const res = await fetch(
        `${API}/Admin/Receptionist/${recDeleteTarget.id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleRecReactivate = async (recepId) => {
    setRecReactivatingId(recepId);
    setRecReactivateError("");
    try {
  ...
  const handleDeleteHospital = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setHospError(
          data.Message || data.message || "Failed to delete hospital.",
        );
        return;
      }
      navigate("/admin");
  ...
  const docPct = dashboard?.maxNumberOfDoctors
    ? (dashboard.numberOfDoctors / dashboard.maxNumberOfDoctors) * 100
    : 0;
  const recPct = dashboard?.maxNumberOfReceptionists
    ? (dashboard.numberOfReceptionists / dashboard.maxNumberOfReceptionists) *
      100
    : 0;
      ...
      <div className="hosp-inner">
        ...
        <div className="hosp-page-header">
          <div>
            ...
            <p className="hosp-page-subtitle">
              Capacity overview and staff management
            </p>
          </div>
        </div>
        ...
        <div className="hosp-hospital-actions">
          <button
            ...
            onClick={() =>
              navigate(`/admin/HospitalInfo/${hospitalId}/UpdateHospital`, {
                state: { hospitalId },
              })
            }
          >
        ...
        <div className="hosp-panel">
          ...
          {docDeleteError && (
            <div className="hosp-alert" style={{ margin: "0.75rem 1.25rem 0" }}>
              {docDeleteError}
            </div>
          )}
          {docReactivateError && (
                ...
                <tbody>
                  {doctors.map((doc) => {
                    ...
                    return (
                      <tr
                        key={doc.id}
                        className={isDeleted ? "hosp-row--deleted" : ""}
                      >
                        <td>
                            ...
                            <div className="hosp-doc-avatar">
                              {doc.profileImageUrl ? (
                                ...
                                <span className="hosp-doc-avatar-initials">
                                  {buildInitials(doc.firstName, doc.lastName)}
                                </span>
                              )}
                            </div>
                        ...
                        <td>
                          <span
                            className={`hosp-badge hosp-badge--${(doc.gender || "").toLowerCase()}`}
                          >
                            {doc.gender || "—"}
                          </span>
                        </td>
                        <td>
                          <div
                            className={`hosp-status-cell ${isActive ? "hosp-status-cell--active" : "hosp-status-cell--inactive"}`}
                          >
                            <span
                              className={`hosp-status-dot ${isActive ? "hosp-status-dot--active" : "hosp-status-dot--inactive"}`}
                            />
            ...
            {recLoading ? (
              ...
              <div className="hosp-empty">
                ...
                <p className="hosp-empty-msg">
                  {recSearch
                    ? "Try a different search term."
                    : "No receptionists assigned to this hospital yet."}
                </p>
              </div>
            ) : (
              <table className="hosp-table">
                  ...
                  {receps.map((rec) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          <span
                            className={`hosp-badge hosp-badge--${(rec.gender || "").toLowerCase()}`}
                          >
                            {rec.gender || "—"}
                          </span>
                        </td>
